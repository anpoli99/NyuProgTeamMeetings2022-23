...
// The idea here is to binary search with a greedy algorithm
// If we're trying to make sure that the sum of strengths is at least
// 'value', we should try to match the strongest person with the weakest
// possible person. This implementation tries all possible pairs of people,
// but prioritizes the weakest people first. 
        ...
        for(int i = 1; i <= 3; i++){ // 1 = beginner, 2 = normal, 3 = expert
            for(int j = 1; j <= 3; j++){
                ...
                if(tmp_cnt[i] > 0 && tmp_cnt[j] > 0 && tmp >= value){
                    chosen_people = {i, j};
                    speed = tmp;
                    goto breakout; // goto can be useful for nested loops
                }
            }
        }
        breakout:
